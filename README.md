...
- **可量化评估**：patient 49 题 38.8/40（97.9%）；doctor 49 题 38.4/40（~90%）；OOB 双模式拦截率/无幻觉率 100%
...
| 测试集 | 模式 | 题数 | 指标 | 结果 |
|--------|------|------|------|------|
| in-scope | patient | 49 | 平均分 / 40 | 38.8（97.0%）✓ |
| in-scope | patient | 49 | 通过率（≥34/40）| 97.9% ✓ |
| in-scope | doctor | 49 | 平均分 / 40 | 38.4（96.0%）✓ |
| in-scope | doctor | 49 | 通过率（≥34/40）| ~90%（API 随机性，两轮 91.8%/87.7%，均在目标范围）✓ |
